...
public class BancoDeDados implements IBancoDeDados {
	...
	private static final String DB_CREATE_TABLE_LISTA_COMPRAS_PRODUTOS = "CREATE TABLE IF NOT EXISTS  lista_compras_produtos (lista_compras_id integer not null, produto_id integer not null, quantidade double, unidade character not null, no_carrinho boolean not null, data_criacao text not null, data_modificacao text not null, preco double not null, PRIMARY KEY(lista_compras_id, produto_id), FOREIGN KEY(lista_compras_id) REFERENCES lista_compras(id) on update cascade on delete cascade, FOREIGN KEY(produto_id) REFERENCES produtos(id) on update cascade on delete restrict);";
